...
**Forms**: forms (`LoginPageForm`, `RegistrationForms`) are currently presentational only — Chakra `Field.Root`/`Input` markup in `LoginPageForm`, plain HTML `<label>`/`<input>`/`<button>` styled via CSS Modules in `RegistrationForms`, with no state wiring, validation, or submit handlers yet, aside from `RegistrationForms` which uses local `useState` to step through a two-step wizard (personal info → address) via dot navigation.
